...
export async function extractTextFromPDF(file: File): Promise<string> {
  try {
    ...
    if (fullText.trim().length < 100) {
      throw new PDFError(
        'SCANNED_PDF',
        'This PDF appears to be scanned or image-based. Please upload the "Policy Wording" document - a separate PDF from your insurer containing coverage details, exclusions, and waiting periods.'
      );
    }
